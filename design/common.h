...
public:
    ...
    virtual Value GetValue(CacheInterface& cache) const = 0;
...
public:
    struct Node {
        std::unordered_set<Position> inputs;
        std::unordered_set<Position> outputs;
    };
...
public:
    ...
    explicit CacheInterface(const SheetInterface& sheet, const GraphInterface& dgraph);
    virtual CellInterface::Value GetValue(const Position& pos);
private:
    void Invalidate();
    const SheetInterface& sheet_;
    const GraphInterface& dgraph_;
    std::unordered_map<Position, Cache> cache_;
};
